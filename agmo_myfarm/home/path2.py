...
def calculate_paths2():
    df = pd.read_excel("C:/Users/jungt/project-name/myagmo_final/agmo_myfarm/data_file/AGMO_Dataset2_output.xlsx")

    path_data1 = df.iloc[:, [4,5]]
    roll_data = df.iloc[:,9].values
    pitch_data = df.iloc[:,10].values
    #print(path_data1)
    ...
    pitch_data_past = pitch_data[:mid_index].copy()
    pitch_data_future = pitch_data[mid_index:].copy()
